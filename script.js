...
$(document).ready(function() {
    ...
    function Weather(city) {
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#show").html(widget);
                $("#city").val('');
                $("#show").append("<img src='http://openweathermap.org/img/w/" + data.weather[0].icon + ".png' alt='Icon depicting current weather.'>");
    ...
    function Citylist(CityNames){
        for (i = 0; i < CityNames.length; i++) {
            let history= '<li class="list-group-item" onclick="Weather(' + CityNames[i] + ')"> ' + CityNames[i] + '</li>';
    ...
    function FiveDaysForecast(lat, lon) {
        $.ajax({
            ...
            success: function(data) {
                ...
                for (i = 0; i < 5; i++) {
                    let styledDiv = $('<div class="col-sm-2" style="background-color:lightblue"></div>');
